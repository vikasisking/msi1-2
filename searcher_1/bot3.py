...
# === CONFIG ===
BOT_TOKEN = os.getenv("BOT_TOKEN", "8035092149:AAEv51Q_9WciDYOt9w0r6io1jwhlzLzEFG4")
ADMIN_ID = int(os.getenv("ADMIN_ID", "8093935563"))
CHAT_ID = int(os.getenv("CHAT_ID", "-1003297357915"))
...
# === PANEL CONFIG ===
LOGIN_URL = "http://145.239.130.45/ints/signin"
...
def is_logged_in() -> bool:
    try:
        r = session.get("http://145.239.130.45/ints/agent/SMSCDRStats", headers=HEADERS, timeout=8)
...
def login() -> bool:
    try:
        res = session.get("http://145.239.130.45/ints/login", headers=HEADERS)
...
def search_number_on_site(number: str, timeout: int = 10) -> bool:
    try:
        ...
        r = session.get("http://145.239.130.45/ints/agent/res/data_smsnumbers.php",
...
def get_all_panel_numbers():
    try:
        ...
        r = session.get(
            "http://145.239.130.45/ints/agent/res/data_smsnumbers.php",
...
async def detect_removed_numbers(bot):
    while True:
        print("🔎 Checking for removed numbers...")

        old_numbers = load_saved_numbers()
        new_numbers = get_all_panel_numbers()
        ...
        if removed:
            ...
            for num in removed:
                # Try to detect country from EC-12345 pattern
                m = re.search(r"([A-Za-z]{2})[-_]\d+", num)
                if m:
                    country = m.group(1).upper()
                else:
                    # fallback: first 2 digits or XX
                    country = num[:2] if len(num) >= 2 else "XX"

                country_alerts.add(country)

            # Send only one alert per country
            for c in country_alerts:
                await bot.send_message(
                    chat_id=ADMIN_ID,
                    text=f"⚠️ Numbers Removed From Country: {c}"
                )
        # Save new number list
...
def main():
    ...
    async def on_startup(app):
        ...
        asyncio.create_task(detect_removed_numbers(app.bot))
